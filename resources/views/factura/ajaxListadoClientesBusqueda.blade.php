        ...
        <tbody class="text-gray-600 fw-semibold">
            @forelse ($clientes as $cli)
                <tr>
                    ...
                    <td>{{ $cli->correo }}</td>
                    <td>
                        <button title="Escoger Cliente" class="btn btn-sm btn-success btn-icon"
                            onclick="escogerCliente('{{ $cli->id }}', '{{ $cli->nombres }}', '{{ $cli->ap_paterno }}', '{{ $cli->ap_materno }}', '{{ $cli->cedula }}', '{{ $cli->nit }}', '{{ $cli->razon_social }}')"><i
